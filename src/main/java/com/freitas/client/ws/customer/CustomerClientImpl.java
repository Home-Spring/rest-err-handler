...
public class CustomerClientImpl extends BaseWebServiceClientImpl implements CustomerClient {
	...
	public Customer saveCustomer(Customer customer) {
		...
		response = restTemplate.execute(getServiceUrl() + "/ws/customer", HttpMethod.POST, requestCallback, responseExtractor);
	...
	public Customer getCustomerByName(String name) {
		if (name == null || name.isEmpty()) {
			throw new CustomerException(ErrorCodeCustomerEnum.INVALID_PARAMS, "A valid name is required");
		}
		...
		response = restTemplate.execute(getServiceUrl() + "/ws/customer?name={name}", HttpMethod.GET, requestCallback, responseExtractor, uriVariables);
//        response = restTemplate.execute(getServiceUrl() + "/ws/customer?name={name}", HttpMethod.POST, requestCallback, responseExtractor, uriVariables);
